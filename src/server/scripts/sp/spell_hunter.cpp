...
class spell_hun_aspect_of_the_hawk: public SpellScriptLoader
{
    ...
    class spell_hun_aspect_of_the_hawk_AuraScript: public AuraScript
    {
        ...
        void CalculateAmount(AuraEffect const * /*aurEff*/, int32 & amount,  bool & /*canBeRecalculated*/) 
        {
            ...
            if (AuraEffect* aurEff = GetCaster()->GetAuraEffect(SPELL_AURA_ADD_PCT_MODIFIER, SPELLFAMILY_HUNTER, 5080, 0))
                coef += aurEff->GetAmount() * 0.01f;

            amount = 23.53f * GetCaster()->getLevel() * coef; 
        }
